/* ============================================================
   ...
   Each section has three parts, all optional:
     - `infographic` — the always-visible static image(s). Two shapes:
         - a single image: { src, alt, caption } — set `src` once you
           have one (build it yourself, or ask for help); leave it
           unset and a dashed placeholder frame renders instead, so
           an unfinished section still shows where the image will go.
         - a set of parallel images (e.g. one per phase), instead of
           one picture: { layout: 'card' | 'vertical', items: [...] }
           — `items` is an array of the same { src, alt, caption }
           shape. 'card' lays them out as a responsive grid, several
           per row; 'vertical' stacks them one per row, each as wide
           as the column allows — pick 'vertical' when an image needs
           the full width to stay legible rather than sitting side by
           side. Defaults to 'card' if `layout` is omitted.
