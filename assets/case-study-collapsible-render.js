...
(function () {
  ...
  /* ------------------------------------------------------------
     ...
     A section whose infographic is itself markup (e.g. a diagram
     authored directly in HTML rather than exported as an image) can
     set `html` instead of `src`: a trusted, pre-authored HTML string
     inserted as-is, no placeholder/caption frame around it. Trusted
     because it's part of this file, same as everything else here —
     never wire this up to anything user-supplied.
     ------------------------------------------------------------ */
  ...
  function infographic(fig, sectionTitle) {
    if (fig && fig.html) {
      return '<figure class="cs-infographic cs-infographic--html">' + fig.html + '</figure>';
    }
    if (fig && fig.items) {
